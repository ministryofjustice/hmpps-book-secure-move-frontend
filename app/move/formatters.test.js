  ...
  describe('#displayDate()', function() {
    context('when input value is a valid date', function() {
      context('with default date format', function() {
        ...
        it('should return default display date format', function() {
          const date = formatters.displayDate('10/10/2010')
          expect(date).to.equal('10 Oct 2010')
        })

        it('should return default date format', function() {
          const date = formatters.displayDate('today')
          expect(date).to.equal('10 Aug 2017')
        })

        it('should return default date format', function() {
          const date = formatters.displayDate('yesterday')
          expect(date).to.equal('9 Aug 2017')
        })
      })

      context('with custom date format', function() {
        it('should return custom date format', function() {
          const date = formatters.displayDate('2010-10-10', 'dd/MM/yyyy')
          expect(date).to.equal('10/10/2010')
        })
      })
    })
    ...
    context('when input value is not a valid date', function() {
      it('should return input value', function() {
        const date = formatters.displayDate('not-a-date')
        expect(date).to.equal('not-a-date')
      })
    })
  })
