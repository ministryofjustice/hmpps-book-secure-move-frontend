...
test('With unfound person', async t => {
  ...
  await t.expect((page.nodes.submitButton as Selector).exists).notOk()
...
test('With existing person', async t => {
  ...
  // Court information
  const courtHearings: any = await createMovePage.fillInCourtHearings()
  await page.submitForm()

  // Risk information
  const riskInformation: any = await createMovePage.fillInReleaseStatus()
  await page.submitForm()

  // Health information
  const healthInformation: any = await createMovePage.fillInSpecialVehicle()
  await page.submitForm()
