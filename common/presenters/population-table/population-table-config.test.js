...
const locationMetaToSpacesLink = function ({ ...rest }) {
  return { ...rest }
}

const populationTableConfig = proxyquire('./population-table-config', {
  './location-meta-to-spaces-link': locationMetaToSpacesLink,
})
  ...
  describe('dayRowConfig', function () {
    ...
    context('rendering row data', function () {
      ...
      context('with no populationIndex', function () {
        ...
        it('should render with href', function () {
          expect(result.hrefPrefix).to.equal('/')
        })

        it('should render with id', function () {
          expect(result.id).to.equal('BEEFCAFE')
        })

        it('should render with spaces free', function () {
          expect(result.space).to.equal(9)
        })
      })
      ...
      context('with populationIndex === 1', function () {
        ...
        it('should render with id', function () {
          expect(result.id).to.equal('FEEDC0DE')
        })

        it('should render with spaces free', function () {
          expect(result.space).to.equal(-3)
        })
      })
    })
  })
