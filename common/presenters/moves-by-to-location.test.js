    ...
    context('when provided with mock moves response', function () {
      ...
      describe('location order', function () {
        it('should order correctly', function () {
          expect(transformedResponse[0].location.title).to.equal('Axminster Crown Court')
        })

        it('should order correctly', function () {
          expect(transformedResponse[1].location.title).to.equal('Barnstaple Magistrates Court')
        })

        it('should order correctly', function () {
          expect(transformedResponse[2].location.title).to.equal('Barrow in Furness County Court')
        })
      })
