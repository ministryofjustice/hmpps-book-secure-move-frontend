  ...
  describe('#callAsMacro()', function () {
    ...
    context('if macro doesn\'t exist', function () {
      ...
      it('should log warning', function () {
        expect(logger.warn).to.be.calledOnceWithExactly(`'${macroName}' macro does not exist`)
      })
  ...
  describe('#getAssetPath()', function () {
    ...
    context('when manifest file doesn\'t exist', function () {
      ...
      it('should log friendly message', function () {
        ...
        expect(logger.error.args[0][0].message).to.equal('Manifest file is not found. Ensure assets are built.')
      })

      it('should log full error', function () {
        expect(logger.error.args[1][0] instanceof Error).to.be.true
        expect(logger.error.args[1][0].message).to.equal('Cannot find module \'path/to/missing/manifest.json\'')
      })
    })
  })
})
